...
- **패널 토글** — 패널 헤더를 클릭해 해당 AI를 켜거나 끌 수 있음 (최소 1개 유지)
- **가로 크기 조절** — 패널 사이 구분선을 드래그해 각 패널 너비 조절
- **세로 크기 조절** — 입력창 위 핸들을 드래그해 입력창 높이 조절
- **패널별 새로고침** — 패널 헤더에 마우스를 올리면 ↺ 버튼으로 해당 패널만 새로고침
- **파일 첨부** — 드래그 앤 드롭 또는 클릭으로 파일 업로드 (파일당 최대 20MB)
- **라이트/다크 테마** — 버튼 하나로 익스텐션 UI와 3개 AI 서비스 테마 동시 전환
- **세션 자동 저장** — 새 대화 URL이 감지되면 자동으로 세션 저장
- **세션 사이드바** — 왼쪽에 항상 표시되는 저장된 세션 목록
...
| 줄바꿈 | `Shift + Enter` |
| AI 켜기/끄기 | 패널 헤더 클릭 |
| 패널 너비 조절 | 패널 사이 세로 구분선 드래그 |
| 입력창 높이 조절 | 입력창 위 가로 핸들 드래그 |
| 패널 개별 새로고침 | 패널 헤더 호버 → `↺` 클릭 |
| 컨트롤바 접기/펼치기 | `▼ / ▲` 버튼 |
| 라이트/다크 테마 전환 | `☀️ Light` / `🌙 Dark` 버튼 |

### 세션 관리

- 전송 후 새 대화 URL이 감지되면 **자동 저장** (~2.5초 후)
- 왼쪽 Sessions 패널에서 저장된 세션 클릭 → 해당 대화로 이동
- **+ 현재 세션 저장** 버튼으로 수동 저장 가능
- 세션 항목에 마우스를 올리면 삭제 버튼 표시

### 파일 첨부

- **📎 파일** 버튼 클릭 또는 창 어디에나 파일 **드래그 앤 드롭**
- 첨부된 파일은 칩 형태로 표시, **✕** 로 개별 제거
- 파일은 다음 메시지 전송 시 함께 전달됨
- 파일당 최대 20MB
...
- Claude는 인코그니토 모드(임시 채팅, 기록 미저장)로 열릴 수 있습니다 — Anthropic이 크로스 오리진 iframe 삽입에 적용한 의도적인 보안 정책으로, 우회가 불가합니다
